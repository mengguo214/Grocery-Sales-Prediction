...
for (i in 1:8){
  ...
  for (j in 1:24){
    # regression tree
    f=as.formula(paste(Store[[i]][j+98],"~",paste(colnames(Store[[k]])[3:98],collapse="+")))
    ...
    # boosted
    #set.seed(1)
    #boost.store = gbm(y_train[,j]~.-Random,store_train,distribution = "gaussian", n.trees=500,shrinkage=0.2,interaction.depth=4)
    #pred.boost=predict(boost.store,newdata=store_test,n.trees=500)
    #RMSE_boosted = rmse(y_test[,j],pred.boost)
